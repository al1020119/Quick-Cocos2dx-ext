...
-- 2017.10.27 added by marvin 
-- @return 有子节点时直接返回当前节点
function ElementNode:getnodecontent(nPreNodeIdx)
	nPreNodeIdx = nPreNodeIdx or 0
	if #self.nodes > 0 then
		local bIdx = self._openend + 1
		local eIdx = self._closestart - 1
		if nPreNodeIdx > 0 then
			bIdx = self.nodes[nPreNodeIdx]._closeend + 1
		end
		if nPreNodeIdx < #self.nodes then
			eIdx = self.nodes[nPreNodeIdx + 1]._openstart - 1
		end
		return string.sub(self.root._text, bIdx, eIdx)
	end
	return string.sub(self.root._text, self._openend + 1, self._closestart - 1)
end
